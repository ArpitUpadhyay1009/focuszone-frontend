...
const LevelUpgradeSystem = () => {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        ...
        const response = await fetch('http://localhost:3001/api/auth/user-level', {
          headers: {
        ...
        const upgradesResponse = await fetch('http://localhost:3001/api/auth/can-level-upgrade', {
          headers: {
        ...
        if (storedTimeStr) {
          ...
          const timeProgress = Math.min(100, (data.timeSpent / data.timeNeeded) * 100);
          setTimeProgress(timeProgress);
        }
      } catch (error) {
      ...
      if (storedTimeStr) {
        ...
        if (!isNaN(storedTime)) {
          setUserData(prev => {
            ...
            const timeProgress = Math.min(100, (updatedTimeSpent / prev.timeNeeded) * 100);
            setTimeProgress(timeProgress);
            return {
              ...prev,
  ...
  const handleUpgrade = async () => {
    if (userData.coins < UPGRADE_COST) {
      toast.error(`Not enough coins! You need ${UPGRADE_COST} coins to upgrade.`);
      return;
    }
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/api/auth/updateLevel', {
        method: 'PATCH',
        headers: {
  ...
  return (
    ...
    >
      <h3 className="text-xl font-semibold mb-4 text-gray-800">Level Progress</h3>
      ...
      <div className="mt-4 mb-2">
        <div className="flex justify-between items-center mb-2">
          <span className="text-sm font-medium text-gray-700">Timer Progress</span>
      ...
      <div className="flex justify-between items-center mt-5">
        ...
        >
          ...
          <div className="flex items-center px-3 py-1 bg-amber-50 rounded-md border border-amber-200">
            ...
            <span className="font-semibold text-amber-600">{userData.coins}</span>
          </div>
        </motion.div>
      </div>
